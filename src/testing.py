...
def listAllItems():
    try:
        allItemDF = pd.read_csv('/items.csv')

    except: return

    allItemNames = []
    for i in range(4, len(allItemDF)):
        allItemNames.append(allItemDF['9'][i])
...
itemList = ['Grade 7 Tincture of Mind', 'Grade 7 Tincture of Dexterity']
itemName = itemList[0]
weeksSlider = 2
returnServer = "Aether"
showMaterials = True
...
totalResults = 1000
maxToGet = 99999
secondsForWeeks = numOfWeeks * 604800
includeMats = "Yes"
includeSales = "Yes"
onlyHQ = "Yes"
...
for name in itemList:
    thisListDF = fetchSalesData(name, hqOnly= True, numOfWeeks= weeksSlider, datacenter = returnServer)
